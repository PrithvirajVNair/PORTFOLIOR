    ...
    return (
        <>
            <div className='fixed w-full z-999'>
                <div className='flex text-white px-5 md:px-10 md:py-0 py-2 max-md:bg-yellow-200/10 max-md:backdrop-blur-[3px] md:mt-5 justify-between md:justify-normal items-center max-md:border max-md:border-yellow-200/10'>
                    <div style={{ fontFamily: '"Bebas Neue", sans-serif' }}>
                        <a className='text-2xl md:text-4xl' href="#home">PVN<span className='text-yellow-200'>.</span></a>
                    </div>
                    <div className='hidden sm:block absolute left-1/2 -translate-x-1/2'>
                        <ul className='hidden md:flex justify-center items-center border border-yellow-200/20 h-15 rounded-lg bg-yellow-200/5' style={{ backdropFilter: 'blur(3px)' }} >
                            {
                                navBarList.map((nav) => (
            ...
            <div className='grid grid-cols-5 text-yellow-200'>
                <div></div>
                <ul className={`md:hidden ${menuOpen && 'border border-yellow-200/10'} backdrop-blur-xs bg-yellow-200/10 text-center rounded-2xl col-span-3 leading-8`}>
                    {
                        menuOpen && (
